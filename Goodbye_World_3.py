...
def create_feedback_loop_video():
    """
    Applies an aggressive pixelation effect repeatedly to the previous frame's output,
    creating a continuous degradation video (feedback loop) until the image is unrecognizable/monochrome.
    """
    ...
    # Get Decay Factor (how fast the image decays)
    while True:
        try:
            # Multiplier: 0.90 is slow, 0.80 is fast. Use a low factor for quick degradation.
            decay_factor = float(
                input("Enter pixel reduction factor (e.95 for slow, 0.80 for aggressive pixelation): "))
            if not (0.75 <= decay_factor < 1.0):
                print("Factor should be between 0.75 and 0.99.")
            else:
                break
        except ValueError:
            print("Invalid input. Please enter a number (e.g., 0.85).")

    # --- 2. Dynamic Naming Scheme ---

    base_name = os.path.splitext(os.path.basename(input_image_path))[0]
    animation_type = 'pixel_collapse'
    ...
    print(f"\n--- Generating {num_frames} Collapse Frames ---")
    ...
    for i in range(num_frames):
        ...
        # --- The Aggressive Pixelation Effect ---

        # 1. Calculate new dimensions for the SHRUNKEN image
        # This is the aggressive part: we shrink the image resolution significantly every frame.
        shrink_w = int(current_img.shape[1] * decay_factor)
        shrink_h = int(current_img.shape[0] * decay_factor)

        # Stop condition: When the image is effectively a single block of color
        if shrink_w < 1 or shrink_h < 1:
            print(f"\nImage completely decayed to ~1 pixel at frame {i}. Stopping loop.")
            num_frames = i
            break

        # 2. Shrink using INTER_NEAREST (creates the coarse pixel block)
        shrunken_img = cv2.resize(current_img, (shrink_w, shrink_h), interpolation=cv2.INTER_NEAREST)

        # 3. Re-enlarge back to the ORIGINAL dimensions
        # This is what makes the small shrunken image visible as huge pixel blocks.
        # Note: We use INTER_NEAREST again to preserve the hard pixel edges.
        current_img = cv2.resize(shrunken_img, (original_width, original_height), interpolation=cv2.INTER_NEAREST)

        # 4. Optional: Subtle Color Shift (introduces psychedelic drift)
        hsv_img = cv2.cvtColor(current_img, cv2.COLOR_BGR2HSV)
        # Shift Hue by 1 (HSV max is 180). This cumulative shift makes the color drift.
